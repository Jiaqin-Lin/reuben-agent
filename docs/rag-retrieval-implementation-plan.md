...
待补齐 (Phase 7-10):
  Phase 7 (Evidence Gates) ──→ Phase 8 (Query Rewrite) ──→ Phase 9 (Parent Block Elevation)
                                                                      │
                                                                 Phase 10 (Rerank)
                                                                      │
                                                                 Phase 11 (集成验证)
...
- [ ] 创建文件 `business/rag/src/main/java/com/reubenagent/rag/service/QueryRewriteService.java`
- [ ] 注解：`@Slf4j @Component @AllArgsConstructor`
- [ ] 注入：`ObjectProvider<ChatModel>`、`RagProperties`
- [ ] 核心方法：`String rewrite(String originalQuery)`
...
- [ ] Prompt 模板：`src/main/resources/prompt/rag-query-rewrite.st`，内容简洁——只做改写，不拆子问题

### 8.2 修改 `RagRetrievalServiceImpl`

- [ ] 在检索前插入：`String rewrittenQuery = rewriteService.rewrite(request.getQuery())`
- [ ] 用 `rewrittenQuery` 替代原 query 传给两个通道
- [ ] `RagRetrieveResponse` 新增字段：`String rewrittenQuery` — 返回改写结果给调用方（可观测）
...
- [ ] 创建文件 `business/rag/src/main/java/com/reubenagent/rag/service/ParentBlockElevationService.java`
- [ ] 注解：`@Slf4j @Component @AllArgsConstructor`
- [ ] 注入：`@Qualifier("ragPgVectorJdbcTemplate") JdbcTemplate`
- [ ] 核心方法：`List<RetrievalResult> elevate(List<RetrievalResult> fusedResults)`
...
- [ ] 防御性：parentBlockId 为 null → 跳过该 result；MySQL 查不到 → 保留原 chunk

### 9.2 修改 `RagRetrievalServiceImpl`

- [ ] RRF 融合后：`List<RetrievalResult> elevated = elevationService.elevate(fusedResults)`
- [ ] 截断 topK 在 elevation **之后**进行（elevate 后数量可能减少）

### 9.3 与 super-agent 的差异

- super-agent 通过 `DocumentKnowledgeService.elevateToParentBlocks()` 实现，依赖 entity 层
- reuben-agent 不能直接依赖 document 模块的 entity/mapper，但可以通过 MyBatis-Plus 连同一 MySQL 库查 `reuben_agent_document_parent_block` 表

**⚠️ 模块隔离问题**：RAG 模块需要访问 MySQL 的 `reuben_agent_document_parent_block` 表。方案：
- 在 RAG 模块定义自己的 `DocumentParentBlockMapper`（只读），连同一数据库
- MyBatis-Plus 的 `BaseMapper` 不关心哪个模块定义的，只要表名和字段对就能查
...
| 8 | **Query Rewrite** | **2h** | ❌ 待补齐 |
| 9 | **Parent Block Elevation** | **2h** | ❌ 待补齐 |
| 10 | **Rerank** | **2h** | ❌ 待补齐 |
| 11 | 补齐后集成验证 | 2h | ❌ |
| | | | |
| **已完成** | | **10h** | |
