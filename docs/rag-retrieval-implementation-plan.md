...
- [ ] 创建文件 `business/rag/src/main/java/com/reubenagent/rag/config/RagElasticsearchConfiguration.java`
- [ ] 注解：`@Configuration @ConditionalOnBean(ElasticsearchClient.class)`
- [ ] 如果 ES Client 已存在（document 模块注册），直接复用；否则注册空 Bean 占位

### 4.2 `KeywordRetrievalChannel`

- [ ] 创建文件 `business/rag/src/main/java/com/reubenagent/rag/service/KeywordRetrievalChannel.java`
- [ ] 注解：`@Slf4j @Component @AllArgsConstructor`
- [ ] 注入：`ObjectProvider<ElasticsearchClient>`（允许 ES 不可用）、`RagProperties`
- [ ] 核心方法：`List<RetrievalResult> retrieve(String query, int topK, Map<String, String> filters)`
  1. 校验 query 非空，ES Client 可用
  2. 构建 `multi_match` 查询：字段 `chunkText`，analyzer `standard`
...
- [ ] 防御性：ES Client 不可用 → log.debug + 空列表（不炸通道）

### 4.3 与 super-agent 的关键差异
...
- [ ] 创建文件 `business/rag/src/main/java/com/reubenagent/rag/service/RrfFusionService.java`
- [ ] 注解：`@Component`（纯函数，无状态，不需要 `@Slf4j`）
- [ ] 核心方法：`List<RetrievalResult> fuse(List<RetrievalResult> vectorResults, List<RetrievalResult> keywordResults, int k, int finalTopK)`
...
- [ ] 创建文件 `business/rag/src/main/java/com/reubenagent/rag/service/IRagRetrievalService.java`
- [ ] 方法签名：`RagRetrieveResponse retrieve(RagRetrieveRequest request)`

### 5.3 `RagRetrievalServiceImpl`

- [ ] 创建文件 `business/rag/src/main/java/com/reubenagent/rag/service/impl/RagRetrievalServiceImpl.java`
- [ ] 注解：`@Slf4j @Service @AllArgsConstructor`
- [ ] 注入：`VectorRetrievalChannel`、`KeywordRetrievalChannel`、`RrfFusionService`、`RagProperties`
- [ ] 核心方法：`retrieve(RagRetrieveRequest request)`
