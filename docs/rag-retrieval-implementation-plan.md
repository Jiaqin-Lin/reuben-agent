...
### 10.1 `RagProperties` 扩展

- [x] 新增内部类 `Rerank`：
  - `boolean enabled = false` — 默认关闭（需要外部 Rerank API）
  - `String url = "https://api.siliconflow.cn/v1/rerank"`
  - `String apiKey = ""` — 通过 `${SILICONFLOW_API_KEY:}` 环境变量注入
...
- [x] 创建文件 `business/rag/src/main/java/com/reubenagent/rag/service/RerankService.java`
- [x] 注解：`@Slf4j @Component`
- [x] 注入：`RagProperties`，构造器中创建 `RestTemplate`（配置 connect/read 超时）
- [x] 核心方法：`List<RetrievalResult> rerank(String query, List<RetrievalResult> candidates)`
  1. 如果 `enabled = false` 或 candidates 为空 → 直接返回 candidates
  2. 构建 HTTP POST：`{model, query, documents: [candidate.chunkText], top_n}` + Bearer auth
  3. 解析返回的 `relevance_score` → 更新 `result.score`，原分数记入 `result.rerankScore`
  4. 按新分数降序排列，source 追加 "+rerank"
  5. 异常 → log.warn + 返回原顺序（降级，不阻塞）

### 10.3 修改 `RagRetrievalServiceImpl`

- [x] Parent Block Elevation 之后：`List<RetrievalResult> reranked = rerankService.rerank(originalQuery, elevated)`
- [x] Rerank 之后截断到 `finalTopK`

### 10.4 与 super-agent 的差异

- [x] super-agent 用 `RestClient` + `DocumentPostProcessor` 接口 —— reuben-agent 直接用 `RestTemplate`，更简单
- [x] super-agent 的 rerank 结果附加 5 个 metadata 字段 —— 精简为 `rerankScore` 单个字段
- [x] 测试：`RerankServiceIntegrationTest` 6 个用例（5 pass + 1 skip 需 API key）
